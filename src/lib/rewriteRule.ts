...
const SLANG_STEMS: Array<{ stems: string[]; out: string[] }> = [
  {
    stems: ["smile", "laugh", "happy", "ecstasy"],
    out: ["lmao", "lmfao", "lol"],
  },
  { stems: ["respect"], out: ["o7"] },
  { stems: ["scare", "horror", "shock", "amaze"], out: ["omg"] },
  { stems: ["what"], out: ["what the hell"] },
  { stems: ["good", "great", "nice"], out: ["goat"] },
  {
    stems: [
      "do",
      "end",
...
function applyRules(
  ...
  // rule 1
  if (tag === "nnp" || tag === "nnps")
    return { ops: properNoun(tok, tokHasSpace), advance: 1 };
  // rule 2
  if (tag === "nn" || tag === "nns")
  ...
  // rule 10
  if (prev === "," || prev === ";")
    return { ops: sentenceBreak(tok, tokHasSpace), advance: 1 };
  // rule 11
  if (prev === ".") return { ops: sentenceStart(tok, tokHasSpace), advance: 1 };
  // default
  return {
    ops: [
      {
...
function properNoun(token: string, hasSpace: boolean): TypingOp[] {
...
function normalNoun(token: string, hasSpace: boolean): TypingOp[] {
  ...
  return buildReplaceOp(token, output, 20, hasSpace);
}
...
function personalPronoun(token: string, hasSpace: boolean): TypingOp[] {
  ...
  return buildReplaceOp(token, output, 50, hasSpace);
}
...
function conj(token: string, hasSpace: boolean): TypingOp[] {
  ...
  return buildReplaceOp(token, output, 60, hasSpace);
}
...
function normalAbbrev(token: string, hasSpace: boolean): TypingOp[] {
  ...
  return buildReplaceOp(token, output, 80, hasSpace);
}
...
function slang(
  ...
  return buildReplaceOp(token, output, 130, hasSpace);
}

// --- grammar based rules ---

// sentence break case
function sentenceBreak(token: string, hasSpace: boolean): TypingOp[] {
...
// sentence start case
function sentenceStart(token: string, hasSpace: boolean): TypingOp[] {
  const shouldAdd = Math.random() < 0.38; // 38% of chance to generate an addon
  if (!shouldAdd) return [{ kind: "type", text: createChunk(token, hasSpace) }];
